        ...
        try:
            with open(file_path, 'r') as file:
                ...
                if end == -1:  # If it's the last entry
                    info = content[start:].strip()
                else:
                    info = content[start:end].strip()
                return info
        except FileNotFoundError:
            return f"Numerology data file 'numerology_data.txt' not found."
        except IOError:
            return f"Error reading the numerology data file."
